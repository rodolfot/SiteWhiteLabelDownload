...
export function MovieBrowser({ movies, categories }: MovieBrowserProps) {
  ...
  const filtered = useMemo(() => {
    ...
    const result = movies.filter((m) => {
      ...
      const matchesGenre = selectedGenres.length === 0 || (m.genre && selectedGenres.some((g) => m.genre.toLowerCase().includes(g.toLowerCase())));
    ...
    return result;
  }, [movies, activeCategory, searchQuery, selectedGenres, yearRange, sortBy, showFavoritesOnly, isFavorite, t.categories.general]);
