...
export function GameForm({ initialData }: GameFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    min_requirements: initialData?.min_requirements || '',
    rec_requirements: initialData?.rec_requirements || '',
  ...
  const [selectedPlatforms, setSelectedPlatforms] = useState<string[]>(
    initialData?.platform ? initialData.platform.split(',').map((p) => p.trim()) : []
  );
  ...
  const togglePlatform = (plat: string) => {
    setSelectedPlatforms((prev) => {
      const updated = prev.includes(plat) ? prev.filter((p) => p !== plat) : [...prev, plat];
      setForm((f) => ({ ...f, platform: updated.join(', ') }));
      return updated;
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (initialData) {
        const { error: updateError } = await supabase.from('games').update(form).eq('id', initialData.id);
        if (updateError) throw updateError;
      } else {
        const { data: existing } = await supabase.from('games').select('id').eq('slug', form.slug).maybeSingle();
        if (existing) form.slug = `${form.slug}-${Date.now().toString(36)}`;
        const { error: insertError } = await supabase.from('games').insert(form).select().single();
        if (insertError) throw insertError;
      }

      logAdminAction({ action: initialData ? 'update' : 'create', entity: 'game', entity_id: initialData?.id, details: form.title });
      router.push('/admin/games');
      router.refresh();
    } catch (err: unknown) {
      console.error('Game save error:', err);
      const msg = err instanceof Object && 'code' in err && (err as { code: string }).code === '23505'
        ? 'Já existe um jogo com este slug. Altere o título ou slug.'
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center gap-4 mb-8">
        <Link href="/admin/games" className="p-2 text-gray-400 hover:text-white hover:bg-surface-700 rounded-lg transition-all">
          <ArrowLeft className="h-5 w-5" />
        </Link>
        <div>
          <h1 className="text-xl font-bold text-white">{initialData ? 'Editar Jogo' : 'Novo Jogo'}</h1>
          <p className="text-gray-400 text-sm mt-0.5">{initialData ? `Editando "${initialData.title}"` : 'Preencha os dados do jogo'}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-surface-800 border border-surface-600 rounded-xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1.5">Desenvolvedor</label>
              <input type="text" value={form.developer} onChange={(e) => updateForm('developer', e.target.value)}
                className="w-full bg-surface-700 border border-surface-500 rounded-lg py-2.5 px-3 text-sm text-white focus:outline-none focus:border-neon-blue transition-all" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1.5">Distribuidora</label>
            ...
            <div>
              ...
              <input type="number" value={form.year} onChange={(e) => updateForm('year', parseInt(e.target.value))} min={1970} max={new Date().getFullYear() + 2}
                className="w-full bg-surface-700 border border-surface-500 rounded-lg py-2.5 px-3 text-sm text-white focus:outline-none focus:border-neon-blue transition-all" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.genre} onChange={(e) => updateForm('genre', e.target.value)} placeholder="ex: Ação, RPG, FPS"
                className="w-full bg-surface-700 border border-surface-500 rounded-lg py-2.5 px-3 text-sm text-white focus:outline-none focus:border-neon-blue transition-all" />
            </div>
        ...
        <div className="bg-surface-800 border border-surface-600 rounded-xl p-6">
          <h2 className="text-white font-semibold text-sm uppercase tracking-wide mb-3">Plataformas <span className="text-gray-500 font-normal normal-case">({selectedPlatforms.length} selecionada(s))</span></h2>
          <div className="flex flex-wrap gap-2">
            {PLATFORMS.map((plat) => (
              <button key={plat} type="button" onClick={() => togglePlatform(plat)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${selectedPlatforms.includes(plat) ? 'bg-neon-blue/20 border border-neon-blue text-neon-blue' : 'bg-surface-700 border border-surface-500 text-gray-300 hover:border-neon-blue hover:text-white'}`}>
                {plat}
              </button>
            ))}
          </div>
        ...
        {/* Requirements */}
        <div className="bg-surface-800 border border-surface-600 rounded-xl p-6 space-y-4">
          <h2 className="text-white font-semibold text-sm uppercase tracking-wide">Requisitos do Sistema</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1.5">Requisitos Mínimos</label>
              <textarea value={form.min_requirements} onChange={(e) => updateForm('min_requirements', e.target.value)} rows={4} placeholder="CPU: ...\nRAM: ...\nGPU: ..."
                className="w-full bg-surface-700 border border-surface-500 rounded-lg py-2.5 px-3 text-sm text-white focus:outline-none focus:border-neon-blue transition-all resize-none" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1.5">Requisitos Recomendados</label>
              <textarea value={form.rec_requirements} onChange={(e) => updateForm('rec_requirements', e.target.value)} rows={4} placeholder="CPU: ...\nRAM: ...\nGPU: ..."
                className="w-full bg-surface-700 border border-surface-500 rounded-lg py-2.5 px-3 text-sm text-white focus:outline-none focus:border-neon-blue transition-all resize-none" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-surface-800 border border-surface-600 rounded-xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" value={form.file_size} onChange={(e) => updateForm('file_size', e.target.value)} placeholder="ex: 50 GB"
                className="w-full bg-surface-700 border border-surface-500 rounded-lg py-2.5 px-3 text-sm text-white focus:outline-none focus:border-neon-blue transition-all" />
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3 justify-end">
          <Link href="/admin/games" className="btn-secondary text-sm">Cancelar</Link>
          <button type="submit" disabled={saving} className="btn-primary flex items-center gap-2 text-sm disabled:opacity-60">
            <Save className="h-4 w-4" /> {saving ? 'Salvando...' : 'Salvar Jogo'}
          </button>
        </div>
      </form>
    </div>
